...
| GeneralMethod | `InfinityCompression/GeneralMethod/` | EPIC_019_GA1 — general method; **EPIC_GS1 (COMPLETE)** — see `Summit/GodelScaleSummitBundle.lean`; also `Galois/TowerEmbeddingProblem.lean`, `RouteD/RiceSelfCertification.lean`, `Summit/UniversalObstructionLaw.lean`, `Summit/CrossDomainDictionaryFull.lean`; prose `specs/IN-PROCESS/EPIC_GS1_M3_MANIFESTO_SYNTHESIS_DRAFT.md`, `EPIC_GS1_LAYMAN_EXPOSITION.md` |
...
| `Galois/GaloisEmbeddingBridge.lean` | EPIC_GS1 Milestone 1: `galois_restrictNormalHom_surjective`, `galois_restrictNormalHom_ker` — Galois restriction ↔ embedding problem (Mathlib re-exports + packaging note); points to `TowerEmbeddingProblem.lean` |
| `Galois/TowerEmbeddingProblem.lean` | EPIC_GS1 M1 completion: `towerEmbeddingProblem`, `towerEmbeddingProblem_solvable_iff_trivial_cocycle` — bundled `EmbeddingProblem` on `Gal(L/K) ↠ Gal(M/K)` for normal towers |
...
| `RouteD/RiceSelfCertification.lean` | EPIC_GS1 Route D completion: `routeD_rice`, `routeD_rice₂`, `routeD_computable_iff_re_and_core` — Rice / Post layer |
...
| `Summit/UniversalObstructionLaw.lean` | EPIC_GS1 Route B unification: `ForgetfulCollision`, `forgetfulCollision_iff_not_injective`, `injective_implies_no_collision` |
...
| `Summit/CrossDomainDictionaryFull.lean` | EPIC_GS1 M5 completion: RCA row (`dictionary_abstract_ic_row_RCA`, `dictionary_abstract_ic_row_non_exhaustion`) + re-imports lightweight hub |
| `Summit/GodelScaleSummitBundle.lean` | EPIC_GS1 summit navigation: `summit_routeC_rca_non_exhaustion`, `summit_routeA_forget_collision`, `summit_milestone1_tower_embedding`, `summit_routeD_halting`, `summit_routeD_rice`, `godel_scale_summit_bundle_ok` |
...
| EPIC_GS1 (Gödel-scale summit program) | **Bundle:** `Summit/GodelScaleSummitBundle.lean`; **M1:** `Galois/TowerEmbeddingProblem.lean`, `Galois/GaloisEmbeddingBridge.lean`; **Route D:** `RouteD/SelfCertificationHalting.lean`, `RouteD/RiceSelfCertification.lean`; **Summit:** `Summit/ReflectiveNonExhaustion.lean`, `Summit/UniversalObstructionLaw.lean`, `Summit/CrossDomainDictionary.lean`, `Summit/CrossDomainDictionaryFull.lean` (+ EPIC_019–022 stack); **prose:** `specs/IN-PROCESS/EPIC_GS1_M3_MANIFESTO_SYNTHESIS_DRAFT.md`, `EPIC_GS1_LAYMAN_EXPOSITION.md` | **Status: COMPLETE** in spec — see `EPIC_GS1_GODEL_SCALE_SUMMIT_PROGRAM_SPEC.md` §5 residual (submission only) |
