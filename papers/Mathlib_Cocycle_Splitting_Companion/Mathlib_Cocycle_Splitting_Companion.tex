...
v4.29.0-rc6. This note is the \emph{technical Mathlib companion}: Mathlib-reviewer-facing
API documentation, not a program overview; see~\cite{SpivackSynthesis} for how the suite
fits together. Phase~1 is the exact prerequisite layer for Mathlib's own documented future
$H^1$/$H^2$ classification theorems.

\textbf{Trust boundary:} This note proposes Mathlib-facing API and Phase~1 lemmas; downstream Mathlib merge status is orthogonal to the internal proof-checking reported here.
\end{abstract}

\SuiteSeriesContext{This manuscript is the \emph{technical Mathlib companion}.}
...
\section{Premises, disagreement coordinates, and trust boundary}\label{sec:premises-ic-mathlib-companion}
\begin{keybox}{Where disagreement can (and cannot) live}
\textbf{Formal spine.}
Phase~1 lemmas (\texttt{splits\_iff\_trivial\_cocycle}, cocycle identities) are claims about the formal code in the cited repository branch, not about future Mathlib merges.

\medskip
\textbf{Premise coordinates (for external readers).}
{\raggedright
\begin{itemize}[leftmargin=2em]
\item \textbf{Contribution vs.\ upstream schedule.} Mathlib maintainers may reorganize namespaces; reviewer-facing API text here is a proposal.
  \emph{Logical effect:} Namespace bikeshedding does not negate the mathematical content at the pinned revision.
\item \textbf{Phase~2 roadmap.} $H^1$/$H^2$ bridges are explicitly future work relative to this note.
  \emph{Logical effect:} Absence of Phase~2 in Mathlib is expected, not a silent failure of Phase~1.
\end{itemize}
\par}
\end{keybox}
...
If $N$ is abelian,
\begin{itemize}[nosep]
\item there is a bijection between $N$-conjugacy classes of\linebreak
  \texttt{(SemidirectProduct.toGroupExtension $\varphi$).Splitting} and\linebreak
  \path{groupCohomology.H1}
\item there is a bijection between equivalence classes of group extensions and\linebreak
  \path{groupCohomology.H2}
\end{itemize}
\end{quote}
...
theorems require. The 2-cocycle identity is proved in genuinely \emph{nonabelian} form, with
factor order arranged so the identity is not a naive commutative transcription---this is
one of the technically delicate points reviewers should expect to see in the proofs.
Concretely, we add:
...
\subsection{Included now vs.\ deferred}

\subsubsection*{In scope for this PR}
\begin{itemize}[nosep]
  \item \texttt{Section.cocycle} --- the $N$-valued 2-cocycle of a set-theoretic section
  \item \texttt{Section.diff} --- pointwise difference between two sections
  \item \texttt{splits\_iff\_trivial\_cocycle} --- splitting criterion
  \item \texttt{Section.conjAct} --- conjugation action on $N$ via sections
  \item \texttt{sectionCocycle\_isMulCocycle\textsubscript{2}\_conj} --- 2-cocycle identity
\end{itemize}

\subsubsection*{Explicitly out of scope for this PR (Phase~2)}
\begin{itemize}[nosep]
  \item $H^1$ conjugacy-class bridge (splittings $\leftrightarrow$ $H^1(G, N)$)
  \item $H^2$ extension-class bridge (extensions $\leftrightarrow$ $H^2(G, N)$)
  \item Full coboundary / change-of-section classification theorem
\end{itemize}

\subsection{Why this belongs in Mathlib now}

\begin{itemize}[nosep]
  \item It fills a documented gap in \texttt{GroupExtension/Defs.lean} (Phase~1 toward the
    stated $H^1$/$H^2$ goals).
  \item It builds only on existing Mathlib group-extension and low-degree cohomology APIs.
  \item It supplies the correct algebraic basis for later $H^1$/$H^2$ classification work.
\end{itemize}

\subsection{Mathematical significance}

Section cocycles and splitting criteria are central classical ingredients in the
structure theory of groups. Their clean formalization is a meaningful library
advance: they are exactly the prerequisite layer for the cohomological
classifications ($H^1$ and $H^2$) already documented as goals in Mathlib. The
2-cocycle identity, proved here in its genuinely nonabelian form (with the
correct factor order for non-commutative kernels), connects the extension-theoretic
cocycle to Mathlib's representation-theoretic cohomology predicates for the first
time.
...
{\small\begingroup\emergencystretch=4em
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.32\textwidth}>{\raggedright\arraybackslash}p{0.66\textwidth}@{}}
...
\texttt{Section.cocycle\_spec}
  & \texttt{S.inl (cocycle $\sigma$ $g_1$ $g_2$) =}\linebreak
    \texttt{$\sigma$\,.toFun $g_1$ *}\linebreak
    \texttt{$\sigma$\,.toFun $g_2$ *}\linebreak
    \texttt{($\sigma$\,.toFun ($g_1$ * $g_2$))$^{-1}$} \\[4pt]
  & \small Defining equation of the cocycle.\linebreak Candidate for \texttt{@[simp]}. \\[6pt]

\texttt{Section.mul\_eq\_cocycle\_mul}
  & \texttt{$\sigma$\,.toFun $g_1$ *}\linebreak
    \texttt{$\sigma$\,.toFun $g_2$ =}\linebreak
    \texttt{S.inl (cocycle $\sigma$ $g_1$ $g_2$) *}\linebreak
    \texttt{$\sigma$\,.toFun ($g_1$ * $g_2$)} \\[4pt]
  & \small Decomposition of section products.\linebreak Rearrangement of \texttt{cocycle\_spec}. \\[6pt]
...
\texttt{Section.cocycle\_mul\_eq}
  & \texttt{cocycle $\sigma$ $g_1$ $g_2$ * cocycle $\sigma$ ($g_1$ * $g_2$) $g_3$ =}\linebreak
    \texttt{conjAct $\sigma$ $g_1$ (cocycle $\sigma$ $g_2$ $g_3$) *}\linebreak
    \texttt{cocycle $\sigma$ $g_1$ ($g_2$ * $g_3$)} \\[4pt]
  & \small The multiplicative 2-cocycle identity (conjugation action).\linebreak
    Required by \texttt{IsMulCocycle\textsubscript{2}} after uncurrying and extracting \texttt{MulDistribMulAction}. \\[6pt]

\texttt{sectionCocycle\_of\_splitting}
  & \texttt{$\forall$ $g_1$ $g_2$,}\linebreak
    \texttt{cocycle s.toSection $g_1$ $g_2$ = 1} \\[4pt]
  & \small The cocycle of a splitting's section is trivial.\linebreak Candidate for \texttt{@[simp]}. \\[6pt]

\texttt{Section.conjAct\_independent}
  & \texttt{[CommGroup N] $\to$}\linebreak
    \texttt{conjAct $\sigma$ g n = conjAct $\sigma'$ g n} \\[4pt]
  & \small For abelian $N$, the conjugation action is independent of section choice. \\[6pt]

\texttt{section\_triple\_product}
  & \texttt{$\sigma$ $g_1$ * $\sigma$ $g_2$ * $\sigma$ $g_3$ =}\linebreak
    \texttt{S.inl (cocycle $\sigma$ $g_1$ $g_2$) * $\sigma$ ($g_1$ * $g_2$) *}\linebreak
    \texttt{$\sigma$ $g_3$} \\[4pt]
...
\path{mul_mul_mul_inv_mem_range_inl} from \path{Basic.lean} directly, rather
than re-deriving the membership fact. Similarly, \texttt{Section.diff} would use
\path{mul_inv_mem_range_inl}. See \Cref{sec:reuse}.
...
The development builds successfully against Mathlib v4.29.0-rc6 (Lean~4 toolchain\linebreak
\path{leanprover/lean4:v4.29.0-rc6}). All files compile with \path{lake build}
