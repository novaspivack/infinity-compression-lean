...
% Normative spec: EPIC_020_ML1 (Paper B).
...
sections, and the multiplicative 2-cocycle identity. One sorry remains in the
2-cocycle identity proof (see \Cref{sec:sorry}); all other theorems are fully
verified. We describe the proposed API surface, design decisions (namespace placement,
...
\codeavailability{%
\url{https://github.com/novaspivack/infinity-compression} — module
\texttt{InfinityCompression.GeneralMethod.GroupExtension.MathlibAdapter}}
...
covers (a) and provides a sorry-bearing proof of~(b), along with the infrastructure
...
\texttt{Section.cocycle\_mul\_eq}
  & \texttt{cocycle $\sigma$ ($g_1$ * $g_2$) $g_3$ * cocycle $\sigma$ $g_1$ $g_2$ = conjAct $\sigma$ $g_1$ (cocycle $\sigma$ $g_2$ $g_3$) * cocycle $\sigma$ $g_1$ ($g_2$ * $g_3$)} \\[4pt]
  & \small The multiplicative 2-cocycle identity with respect to the conjugation action.
    ...
    uncurrying and extracting the \texttt{MulDistribMulAction}).
    \textbf{Contains 1 sorry} (see \Cref{sec:sorry}). \\[6pt]
...
\texttt{sectionDiff}, \texttt{sectionConjAct}) at the top level of the
\texttt{InfinityCompression.GeneralMethod} namespace. For the Mathlib PR, these would
be renamed to use dot notation under \texttt{GroupExtension.Section} and placed in a
new file \texttt{Mathlib/GroupTheory/GroupExtension/Cocycle.lean}.
...
%=======================================================================
\section{Toward $H^1$ and $H^2$: Phase~2 Roadmap}\label{sec:roadmap}
%=======================================================================

\subsection{Phase~2a: The $H^1$ bridge}
...
\subsection{Phase~2b: The $H^2$ bridge}
...
\item \texttt{Section.cocycle\_mul\_eq}: The 2-cocycle identity (with sorry).
...
\midrule
Phase 1 (this PR) & Cocycle + splitting criterion & Complete (1 sorry) \\
...
\texttt{leanprover/lean4:v4.18.0-rc2}). All files compile with \texttt{lake build}
with no errors beyond the single disclosed sorry.

\subsection{The sorry disclosure}\label{sec:sorry}

There is exactly \textbf{one sorry} in the development, located in
\texttt{CocycleIdentity.lean} at the theorem
\texttt{sectionCocycle\_isMulCocycle\textsubscript{2}\_conj} (proposed Mathlib name:
\texttt{Section.cocycle\_mul\_eq}).
...
theorem sectionCocycle_isMulCocycle₂_conj
  ...
  conv_rhs =>
    rw [show g₁ * (g₂ * g₃) = g₁ * g₂ * g₃
          from (mul_assoc g₁ g₂ g₃).symm]
  sorry
\end{lstlisting}

\textbf{Mathematical status.} The theorem is mathematically standard: both sides of the
2-cocycle identity, when mapped through \texttt{S.inl} and right-multiplied by
$\sigma(g_1 g_2 g_3)$, reduce to $\sigma(g_1) \cdot \sigma(g_2) \cdot \sigma(g_3)
\cdot \sigma(g_1 g_2 g_3)^{-1}$ after cancellation of intermediate section values.

\textbf{Why the sorry remains.} After the \texttt{rw} steps, the goal is an equality
in $E$ involving products of section values and their inverses. The \texttt{group}
tactic treats section values $\sigma(g_i)$ as free generators and cannot perform the
necessary cancellations (e.g., $\sigma(g_1 g_2)^{-1} \cdot \sigma(g_1 g_2) = 1$).
Closing the sorry requires a manual chain of \texttt{mul\_assoc}, \texttt{mul\_inv\_cancel},
and \texttt{inv\_mul\_cancel\_left} rewrites---approximately 15--20 steps of associativity
and cancellation management. This is purely tactic engineering, not mathematical
difficulty.

\textbf{Options for closing:}
\begin{enumerate}[nosep]
\item \textbf{Manual rewrite chain}: Write out the explicit associativity and
  cancellation steps. Tedious but straightforward.
\item \textbf{Congruence approach}: Factor the proof through intermediate lemmas that
  isolate each cancellation step, then compose.
\item \textbf{Defer to Phase~2}: The sorry does not block the splitting criterion or
  any other theorem in Phase~1. It blocks only the formal connection to
  \texttt{IsMulCocycle\textsubscript{2}}, which is a Phase~2 dependency.
\end{enumerate}

\textbf{Recommendation for the PR}: Option~1 (manual rewrite chain) is the most
conservative choice. If a reviewer prefers, the theorem can be excluded from the
initial PR and added in a follow-up once the tactic proof is complete.
...
/-
  ...
  * `GroupExtension.Section.cocycle_mul_eq`
  ...
  This module is Phase 1 toward completing the H¹/H²
...
content has been verified by the Lean type checker (modulo the disclosed sorry).
