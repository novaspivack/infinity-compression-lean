...
\textbf{Trust boundary:} This is a twelve-tranche portability survey in Lean; it is not a universality theorem over all mathematical settings and does not subsume APS abstract-indexed separation results.
\end{abstract}

\keywords{formal verification, positive closure, fibers, external validation,
  Mathlib, Lean~4, Infinity Compression architecture}

\amsclassification{03B70, 18A30, 68V15}

\codeavailability{%
  \url{https://github.com/novaspivack/infinity-compression-lean};
  external-validation tranches under \texttt{InfinityCompression/} (no import of the flagship \texttt{IC} spine per tranche design);
  \texttt{lake build}; see \texttt{lakefile.lean} and repository \texttt{MANIFEST.md}.}

\section{Premises, disagreement coordinates, and trust boundary}\label{sec:premises-ic-external-validation}
\begin{keybox}{Where disagreement can (and cannot) live}
\textbf{Formal spine.}
Tranche verdicts (\textbf{STRONG} / \textbf{MODERATE}) reference machine-checked bundles in the stated Lean modules, not informal analogy alone.

\medskip
\textbf{Premise coordinates (for external readers).}
{\raggedright
\begin{itemize}[leftmargin=2em]
\item \textbf{Transfer vs.\ flagship.} Tranches deliberately avoid importing the internal flagship spine; conclusions concern portability of a \emph{pattern}, not identity with NEMS paper claims.
  \emph{Logical effect:} Rejecting one tranche does not automatically refute the flagship internal-origins paper.
\item \textbf{Rubric, not omnicoverage.} Twelve families evidence export of an architectural lens; the paper explicitly disclaims a single theorem quantifying over all mathematics.
  \emph{Logical effect:} Negative search outside the tranche list is independent of this validation's internal logic.
\end{itemize}
\par}
\end{keybox}
...
The benchmark imports only\linebreak
\path{Mathlib/Data/Setoid/Basic.lean} and\linebreak
\path{Mathlib/Logic/Function/Basic.lean}.

\subsection{Lean formalization and main bundle}\label{sec:t1-lean}

All definitions and theorems live in\linebreak
\path{InfinityCompression/Validation/QuotientFiberBenchmark.lean} (namespace
\path{InfinityCompression.Validation}). Layering: \path{BareQuotient},
\path{EnrichedCarrier}, \path{forgetToQuotient}, \path{QuotientFiber},
...
Given \texttt{MulAction G X}, the orbit relation \path{same_orbit} yields a
\texttt{Setoid} and quotient \path{BareOrbitQuotient}; the collapse map is
\path{to_orbit_quotient}. Module:\linebreak\path{OrbitRelationQuotientFiberBenchmark.lean}.
...
\textbf{Lean name} & \textbf{One-line reading} \\
\midrule
\path{same_orbit} & orbit relation $\exists g,\ g \cdot x = y$. \\
\path{grp_action_orbit_setoid} & \texttt{Setoid} for the orbit relation. \\
\path{to_orbit_quotient} & \texttt{Quotient.mk} from \texttt{X} to orbit space. \\
\path{to_orbit_quotient_surjective} & surjectivity. \\
\path{quotient_out_rightInverse_to_orbit_quotient} & \texttt{Quotient.out} as \texttt{RightInverse}. \\
\path{canonical_orbit_quotient_fiber_witness} & canonical witness in each fiber. \\
\bottomrule
\end{tabular}
...
\path{forgetToPushout_surjective} & surjectivity. \\
\path{quotientOut_rightInverse_forgetToPushout} & \texttt{Quotient.out} as \texttt{RightInverse}. \\
...
\path{classifier_fiber_nonempty_of_mem} & nonempty fiber when \texttt{x} lies in \texttt{S}. \\
\path{subtypeVal_surjectiveOn} & surjectivity of \texttt{Subtype.val} under global membership. \\
\path{classifierSection_rightInverse_subtypeVal} & \texttt{RightInverse} for the section. \\
\bottomrule
\end{tabular}
...
\path{forgetPairToLocalization_surjective} & pair-map surjectivity. \\
\path{sec_rightInverse_forgetPairToLocalization} & \texttt{IsLocalization.sec} as \texttt{RightInverse}. \\
\path{canonicalLocalizationPairFiberWitness} & canonical pair in each fiber. \\
\bottomrule
\end{tabular}
...
\path{pullbackSection_rightInverse_pullbackFst} & chosen section is \texttt{RightInverse}. \\
\bottomrule
\end{tabular}
...
\texttt{Nat.succ} is not surjective; fibers over \texttt{n > 0} are nonempty singletons.\linebreak
\path{NonSurjectiveSuccFiberBenchmark.lean} records \texttt{¬\,HasRightInverse\,succ} and\linebreak
\path{pred_leftInverse_succ}.
...
\midrule
T1 & quotient & \path{forgetToQuotient} / \path{Quotient.mk} & equivalence classes & \path{Quotient.out} (canonical) \\
T2 & $X$ & \path{forgetFirst} & second coordinate $y : Y$ & \path{productSection}\,y0 (parametric) \\
T3 & $B$ & \path{Sigma.fst} & dependent $E\,b$ & \path{sigmaSection} (\texttt{noncomputable}) \\
T4 & \path{Bool} & \path{forgetSumIsLeft} & tagged \path{Sum} values & \path{sumSection} (parametric) \\
T5 & $\alpha$ & \path{Subtype.val} & proof data (singleton fibers) & \path{subtypeSection} \\
T6 & orbit quotient & \path{to_orbit_quotient} & orbit classes & \path{Quotient.out} (canonical) \\
T7 & $R/I$ & \path{forgetToIdealQuotient} & cosets mod $I$ & \path{Quotient.out} (canonical) \\
T8 & pushout & \path{forgetToPushout} & \path{EqvGen} classes & \path{Quotient.out} (canonical) \\
T9 & $\alpha$ & \path{Subtype.val} on \path{S} & membership fibers & \path{classifierSection} \\
T10 & localized ring & \path{forgetPairToLocalization} & denominator pairs & \path{IsLocalization.sec} \\
T11 & $A$ & \path{pullbackFst} & compatible $B$-legs & \path{pullbackSection} (\texttt{noncomputable}) \\
T12 & \path{Nat} & \path{Nat.succ} & predecessor fibers & \path{Nat.pred} (left inverse only) \\
\bottomrule
\end{tabular}%
}
...
We formalize this in three Lean modules:\linebreak
\path{GeneralMethod/GroupExtension/FiberArchitecture.lean},\linebreak
\path{GeneralMethod/GroupExtension/SchurZassenhaus.lean}, and\linebreak
\path{GeneralMethod/GroupExtension/LocalGlobal.lean},\linebreak
...
\path{same_orbit} & orbit relation from a \texttt{MulAction}. \\
\path{grp_action_orbit_setoid} & \texttt{Setoid} for orbit equivalence. \\
\path{BareOrbitQuotient} & orbit-space quotient type. \\
\path{to_orbit_quotient} & collapse map \texttt{Quotient.mk}. \\
\path{OrbitQuotientFiber} & fiber over an orbit point. \\
\path{orbit_quotient_fiber_nonempty} & nonempty fibers. \\
\path{to_orbit_quotient_surjective} & surjectivity. \\
\path{to_orbit_quotient_hasRightInverse} & \texttt{HasRightInverse}. \\
\path{quotient_out_rightInverse_to_orbit_quotient} & \texttt{Quotient.out} as \texttt{RightInverse}. \\
\path{canonical_orbit_quotient_fiber_witness} & canonical witness in each fiber. \\
...
\path{forgetToIdealQuotient} & \texttt{Ideal.Quotient.mk I}. \\
\path{IdealQuotientFiber} & fiber over a coset in $R/I$. \\
\path{ideal_quotient_fiber_nonempty} & nonempty fibers. \\
\path{forgetToIdealQuotient_surjective} & surjectivity. \\
\path{forgetToIdealQuotient_hasRightInverse} & \texttt{HasRightInverse}. \\
\path{idealQuotientOut_rightInverse_forgetToIdealQuotient} & \texttt{Quotient.out} as \texttt{RightInverse}. \\
\path{canonicalIdealQuotientFiberWitness} & canonical witness in each fiber. \\
...
\path{classifierSection_rightInverse_subtypeVal} & \texttt{RightInverse} law. \\
