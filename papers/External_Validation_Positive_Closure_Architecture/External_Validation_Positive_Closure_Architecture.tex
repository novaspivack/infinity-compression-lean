...
\large A Multi-Tranche Survey: Quotients, Products, and Dependent Sums in Lean}
\author{Nova Spivack}
\date{2026}
...
\textbf{consolidated machine-checked external validation}: three independent tranches of
...
\texttt{Sigma.fst} (bundle projection; dependent generalization of T2). The contribution is
not a catalog of new classical theorems; it is a demonstration that the same architectural
lens organizes \emph{all three} external families and yields strengthened, reusable proof
...
might be idiosyncratic; this document \emph{consolidates} that quotient tranche with two
additional standard constructions---Cartesian products and dependent sums---so that the
...
formalized in \IC{}'s types. We claim that, in Lean, on three concrete external families,
...
\bottomrule
\end{tabular}
\caption{Side-by-side view: three instantiations of the same architectural schema.}
\label{tab:synth}
\end{table}
...
T2. In all three cases the \textbf{STRONG} rubric applies for the same reason: the
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{External validation (this paper):} machine-checked demonstrations that the
    same pattern applies \emph{outside} the flagship modules, on quotients, products, and
    dependent sums.
...
Three finite families do not exhaust all closure or fibration patterns. Natural extensions
