...
\begin{abstract}
\raggedright
We present the first machine-checked construction of the homotopy data for Quillen's Theorem~A
...
\textbf{Trust boundary:} The formalization stops where noted at simplicial homotopy owing to Mathlib coverage; classical mathematical steps called ``standard'' are not additionally machine-checked.
\end{abstract}
...
\section{Premises, disagreement coordinates, and trust boundary}\label{sec:premises-ic-quillen}
\begin{keybox}{Where disagreement can (and cannot) live}
\textbf{Formal spine.}
Nerve constructions, terminal-object connectivity lemmas, and the Quillen-A packaging cited here are checked in the Lean paths referenced by this repository.

\medskip
\textbf{Premise coordinates (for external readers).}
{\raggedright
\begin{itemize}[leftmargin=2em]
\item \textbf{Homotopy gap.} Edge-level homotopy data are formalized; full simplicial homotopy awaits Mathlib infrastructure.
  \emph{Logical effect:} Extending the formalization may strengthen the discharge without changing vertex-level lemmas already proved.
\item \textbf{Series role.} This paper is the topology discharge in a multi-paper program; algebra/arithmetic discharges are separate manuscripts.
  \emph{Logical effect:} One discharge route does not automatically carry the strength of another.
\end{itemize}
\par}
\end{keybox}
...
This paper presents the first machine-checked construction of the homotopy data for this result in
Lean~4 with Mathlib. The final step (composition homotopic to identity via 	exttt{SSet.Homotopy}) is left for future work once Mathlib's simplicial homotopy API is in place. The formalization consists of seven modules totaling
...
knowledge, no prior machine-checked construction of the homotopy data for Quillen's Theorem~A
exists in any proof assistant (confirmed April 2026 with J.~Riou, who plans a full formalization once further
Mathlib infrastructure is upstreamed). Mathlib provides the nerve construction for categories
...
We have presented a machine-checked construction of the homotopy equivalence
...
splitting, and descent infrastructure. To our knowledge, this is the first machine-checked construction of homotopy data
for Quillen's Theorem~A; a full formalization completing the simplicial homotopy step is future work, and
...
definitions (\path{QuillenAData}, \path{EmbeddingProblem},
\path{DoublingFiber}, \path{TwoTorsionSet}) and theorems
(\path{quillenAData}, \path{solvable_iff_trivial_cocycle},
\path{nerve_connected_to_terminal}) that build on and extend Mathlib's
