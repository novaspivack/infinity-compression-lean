...
only if some section has trivial cocycle.  We also state the 2-cocycle identity
connecting our construction to Mathlib's \IsMulCocycle{} predicate (with one
tactic-level gap remaining in the proof), establish section-independence of the
splitting question, and formalize restriction to subgroups with a local-to-global
obstruction principle.  The work takes a first step toward documented TODOs in
Mathlib's \lean{GroupExtension} module regarding the cohomological classification
of extensions.  All results except one tactic-level \lean{sorry} in the 2-cocycle
identity proof are fully machine-checked.
\end{abstract}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Conjugation action and 2-cocycle identity} (\S\ref{sec:conjact}):
    ...
    via the conjugation action, proving the identity up to one tactic-level
    \lean{sorry}.
...
All results compile against Mathlib with zero \lean{sorry} except for
one tactic-level \lean{sorry} in the proof of
\lean{sectionCocycle\_isMulCocycle\textsubscript{2}\_conj} (the 2-cocycle identity,
\S\ref{sec:cocycle-identity}).  The mathematical content of that identity is
standard; the remaining gap is purely tactic-engineering (see the discussion in
\S\ref{sec:cocycle-identity}).
...
\begin{equation}\label{eq:cocycle-identity}
  \cocy_\sigma(g_1 g_2, g_3) \cdot \cocy_\sigma(g_1, g_2) \;=\;
...
performing associativity and cancellation in $E$.  The formalization unfolds
\lean{sectionCocycle\_spec} for all four cocycle terms and
\lean{inl\_sectionConjAct} for the conjugation, then requires a chain of
rewriting steps in the group $E$.

\paragraph{Proof status.}
\textbf{This theorem is not fully machine-checked.}  The current formalization
contains one \lean{sorry} at the final rewriting step, meaning Lean accepts the
theorem statement but the proof is incomplete.  The proof proceeds by injectivity
of $\iota$: both sides are expanded via \lean{sectionCocycle\_spec} and
\lean{inl\_sectionConjAct}, and the associativity $g_1 \cdot (g_2 \cdot g_3) =
(g_1 \cdot g_2) \cdot g_3$ is rewritten.  After these steps, both sides reduce
to the same expression $\sigma(g_1) \cdot \sigma(g_2) \cdot \sigma(g_3) \cdot
\sigma(g_1 g_2 g_3)^{-1}$, but the remaining chain of cancellations in $E$ is
not yet discharged.  The gap is purely tactic-engineering: Lean~4's \lean{group}
tactic treats section values $\sigma(g_i)$ as free generators and cannot perform
the required cancellations of intermediate terms like
$\sigma(g_1 g_2)^{-1} \cdot \sigma(g_1 g_2)$.  Closing this gap requires either
a manual chain of \lean{mul\_assoc} / \lean{inv\_mul\_cancel\_left} rewrites or a
custom tactic for cancellation in groups with distinguished subexpressions.
...
and the 2-cocycle that measures the gap.  The central fully-verified
results---the splitting criterion (\lean{splits\_iff\_trivial\_cocycle}) and the
local-to-global obstruction principle---together with the 2-cocycle identity
(stated, with one tactic-level gap remaining in the proof) take a first step
toward documented gaps in Mathlib's formalization of group extension theory.
...
The most immediate next step is completing the 2-cocycle identity proof (closing the
one remaining \lean{sorry}).  Beyond that, the path toward Mathlib's $H^2$ TODO
requires:
...
All results are fully machine-checked unless marked with $\dagger$ (one
\lean{sorry}).
...
Thm.~\ref{thm:cocycle-identity}$^\dagger$ &
...
\noindent
$^\dagger$\,Contains one tactic-level \lean{sorry}; the theorem statement is
accepted by Lean but the proof is incomplete.  See the discussion in
\S\ref{sec:cocycle-identity}.
