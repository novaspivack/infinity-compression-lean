...
criterion, and embedding-problem equivalence for group extensions in
Lean~4~\cite{Lean4}, built on Mathlib's \lean{GroupExtension}
API~\cite{Mathlib,Yamazaki2024}. The formalization is organized around a
\emph{fiber architecture} that decomposes extensions into layers: fibers over
quotient elements ($N$-torsors), set-theoretic sections (always exist), and
homomorphic splittings (may not exist). The central results are: (1)~the
\emph{splitting criterion}---an extension splits if and only if some section has
trivial cocycle; (2)~the \emph{embedding-problem equivalence}---an abstract
embedding problem (the central lifting problem in inverse Galois theory) is
solvable if and only if the associated extension splits, if and only if the
cocycle obstruction vanishes; (3)~the \emph{2-cocycle identity} connecting our
construction to Mathlib's \IsMulCocycle{} predicate; and (4)~a
\emph{local-to-global obstruction principle} via restriction to subgroups. The
work fills documented gaps in Mathlib's \lean{GroupExtension} module and takes a
first step toward the cohomological classification of extensions. Beyond
cocycles and Mathlib gap-filling, this paper is the \emph{algebraic and arithmetic
engine} of the summit program: embedding problems, the tower/arithmetic bridge to
inverse Galois theory, and obstruction packaged as fiber/section data. Companion
work discharges the topological instance (Quillen's Theorem~A)~\cite{SpivackQuillen};
the program map is~\cite{SpivackSynthesis}. In one line: this paper supplies the
\emph{algebraic and arithmetic discharge} of the common fiber/section/obstruction architecture,
not only a Mathlib-shaped formalization milestone. All results are fully machine-checked with zero \lean{sorry}.

\textbf{Trust boundary:} Mathlib API surfaces and namespace paths are version-sensitive; this PDF is keyed to the cited Mathlib/Lean pins in the repository.
\end{abstract}

\SuiteSeriesContext{This manuscript is the \emph{algebraic and arithmetic engine}.}
...
\codeavailability{All Lean source files are available in the supplementary
  material under\linebreak
  \path{InfinityCompression/GeneralMethod/GroupExtension/} and\linebreak
  \path{InfinityCompression/GeneralMethod/Descent/}.
  The repository will be made publicly available upon publication.}

\section{Premises, disagreement coordinates, and trust boundary}\label{sec:premises-ic-group-ext}
\begin{keybox}{Where disagreement can (and cannot) live}
\textbf{Formal spine.}
Cocycle identities, splitting criteria, and embedding-problem bridges are theorems in named Lean modules under \path{InfinityCompression/GeneralMethod/}.

\medskip
\textbf{Premise coordinates (for external readers).}
{\raggedright
\begin{itemize}[leftmargin=2em]
\item \textbf{Mathlib evolution.} This paper both fills documented gaps and aligns with Mathlib's \texttt{GroupExtension} API as pinned.
  \emph{Logical effect:} Port conflicts across Mathlib versions are engineering issues, not automatic falsification of the mathematical claims at the pinned revision.
\item \textbf{Publication timing vs.\ code.} Public repository availability may lag the PDF; the formal statements remain the authority.
  \emph{Logical effect:} Request or pin the artifact revision cited in correspondence.
\end{itemize}
\par}
\end{keybox}
...
group extensions and \lean{groupCohomology.H2} (which is also stated as a TODO in\linebreak
\path{GroupCohomology/LowDegree.lean}).''
...
\paragraph{Three strands of the contribution.}
\begin{enumerate}[label=(\arabic*), leftmargin=2em]
  \item \textbf{Library gap addressed:} explicit Mathlib TODOs for the cohomological bridge
    motivate Phase~1 (cocycle, splitting criterion, identity).
  \item \textbf{Theorem package:} the fiber architecture, splitting iff trivial cocycle,
    local-to-global obstruction, and embedding-problem equivalences are fully formalized.
  \item \textbf{Arithmetic interpretation:} abstract embedding problems are packaged as
    extension splitting with kernel-valued cocycle obstruction---the arithmetic face of the
    same architecture used in the flagship certification story.
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Embedding problems as extension splitting} (\S\ref{sec:embedding}):
    We formalize abstract embedding problems and prove
    \lean{solvable\_iff\_splits} and \lean{solvable\_iff\_trivial\_cocycle},
    connecting the central lifting problem of inverse Galois theory to
...
\paragraph{Place in a broader series.}
This paper is the \emph{algebraic and arithmetic engine} of the program: it isolates
the cohomological group-extension layer, the embedding-problem equivalence (abstract
inverse Galois lifting as extension splitting), and the arithmetic bridge (tower
embedding formulations connecting Galois-theoretic data to cocycle obstructions).
It should be read alongside the internal origin theorem~\cite{SpivackIC} and the
transferability survey~\cite{SpivackValidation}; topology is discharged
in~\cite{SpivackQuillen}; the unified map and summit claim are~\cite{SpivackSynthesis,SpivackSummit}.

\subsection{Embedding problems, arithmetic, and summit role}\label{sec:intro-summit}
Classical inverse Galois theory organizes ``realizing'' a group as a Galois group
as an \emph{embedding problem}: solvability is a lifting condition on extensions.
Our formalization makes that identification explicit: an embedding problem is solvable
if and only if a designated group extension splits, if and only if the associated
section cocycle is trivial (\S\ref{sec:embedding}). That packages the same
fiber/section/obstruction pattern used in the flagship certification story---here with
obstructions valued in kernel data and with a documented path toward Galois towers.
This is not a side remark on Mathlib infrastructure; it is the arithmetic face of the
common architecture.

\paragraph{Proof status.}
The formalization comprises twelve Lean modules totaling approximately 1500 lines
of code: seven core group extension modules, two supplementary modules
(\path{NonAbelianObstruction.lean}, \path{SchurMultiplier.lean}), an adapter
module (\path{MathlibAdapter.lean}), an embedding-problem module
(\path{EmbeddingProblem.lean}), and a module for faithfully flat descent.
All results compile against Mathlib v4.29.0-rc6 with zero \lean{sorry}.
...
Section~\ref{sec:embedding} formalizes embedding problems as extension splitting.
...
structures in\linebreak\path{Mathlib/GroupTheory/GroupExtension}:
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \lean{GroupExtension.conjAct}: the conjugation action $E \to^*
    \Aut(N)$, defined via \lean{MulAut.conjNormal}. Concretely, writing $c$ for \lean{conjAct},\linebreak
    $c(e)(n) = \iota^{-1}(e \cdot \iota(n) \cdot e^{-1})$;
    the key specification lemma is \lean{inl\_conjAct\_comm}:
    $\iota(c(e)(n)) = e \cdot \iota(n) \cdot e^{-1}$.
...
%=======================================================================
\section{Embedding Problems as Extension Splitting}\label{sec:embedding}
%=======================================================================

An \emph{embedding problem} in Galois theory asks: given a Galois extension
$L/K$ with group $G$ and a surjection $\varphi : H \twoheadrightarrow G$, does
there exist a Galois extension $F/K$ with group $H$ extending $L/K$? The kernel
$N = \ker(\varphi)$ gives a group extension $1 \to N \to H \to G \to 1$, and a
solution is a homomorphic section $\psi : G \to H$ with
$\varphi \circ \psi = \id$---exactly a splitting of the extension.

We formalize this connection at the abstract group-theoretic level (not yet
connected to actual Galois extensions via Mathlib's \lean{IntermediateField}).

\begin{definition}[Embedding problem]\label{def:embedding}
An \lean{EmbeddingProblem G H} consists of a surjective group homomorphism
$\varphi : H \to G$. The associated group extension is
$1 \to \ker(\varphi) \to H \xrightarrow{\varphi} G \to 1$, constructed as
\lean{ep.toGroupExtension}.
\end{definition}

\begin{theorem}[Solvability as splitting]\label{thm:solvable-splits}
An embedding problem is solvable if and only if the associated group extension
splits:
\begin{lstlisting}
theorem solvable_iff_splits (ep : EmbeddingProblem G H) :
    Nonempty ep.Solution <-> Nonempty ep.toGroupExtension.Splitting
\end{lstlisting}
\end{theorem}

\begin{theorem}[Solvability as trivial cocycle]\label{thm:solvable-cocycle}
Combining with the splitting criterion, an embedding problem is solvable if and
only if the associated extension has a section with trivial cocycle:
\begin{lstlisting}
theorem solvable_iff_trivial_cocycle (ep : EmbeddingProblem G H) :
...
\paragraph{Arithmetic face of the architecture.}
Solvability of an embedding problem is not a primitive predicate: it is a \emph{liftability}
question controlled by kernel-valued obstruction---exactly the same fiber/section pattern as
elsewhere in the suite, with the cocycle as the explicit witness that splitting has failed or
succeeded. This is the arithmetic discharge of the common architecture at the abstract
group-theoretic level.

\noindent
This connects the Galois-theoretic lifting question directly to the cohomological
obstruction from group extension theory. The architecture decomposes the problem
into layers: the bare layer (known Galois group $G$), the enriched layer (target
group $H$), the fiber (kernel $N$), and the obstruction (cocycle class). To our
knowledge, this is the first Lean formalization connecting embedding problems to
group extension splitting with cocycle obstructions.

\paragraph{Scope (arithmetic bridge).}
Abstract embedding problems and their equivalence with splitting/trivial cocycle are
formalized here. The repository also contains a \emph{tower} instance
(\path{TowerEmbeddingProblem.lean}: \path{towerEmbeddingProblem} and\linebreak
\path{towerEmbeddingProblem_solvable_iff_trivial_cocycle}) packaging normal
\lean{IntermediateField} data; this paper focuses on the abstract group-theoretic engine and
cites the tower layer as companion infrastructure rather than reproducing it in full detail.
...
We have presented a machine-checked formalization of the section 2-cocycle,
splitting criterion, embedding-problem equivalence, and local-to-global
obstruction for group extensions in Lean~4. The central theorem package consists
of: the splitting criterion \lean{splits\_iff\_trivial\_cocycle} (an extension
splits iff some section has trivial cocycle); the embedding-problem equivalence
\lean{solvable\_iff\_trivial\_cocycle} (an embedding problem is solvable iff the
associated extension splits iff the cocycle vanishes); the 2-cocycle identity
\lean{sectionCocycle\_isMulCocycle\textsubscript{2}\_conj} (connecting to
Mathlib's low-degree cohomology predicates); and the local-to-global obstruction
principle \lean{local\_nonsplit\_obstructs\_global}. All results are fully
machine-checked with zero \lean{sorry}.

\paragraph{What this means methodologically.}
The fiber architecture turns the gap between set-theoretic existence and algebraic
structure into an explicit intermediate invariant: the 2-cocycle. This invariant
is not merely a classification tool; it is the precise obstruction whose vanishing
is equivalent to splitting, and whose connection to embedding problems bridges
group extension theory to inverse Galois theory. The combination with faithfully
flat descent (conservative forgetful map, no obstruction) demonstrates that the
architecture accommodates a range of algebraic phenomena, from non-conservative
to conservative forgetful maps.

\paragraph{Program closure.}
Readers approaching the Infinity Compression paper suite should treat this manuscript as
the discharge of algebra and (abstract) arithmetic obstruction into standard group theory,
with~\cite{SpivackSynthesis} supplying the cross-domain dictionary and~\cite{SpivackSummit}
stating the summit-level reflective non-exhaustion claim.

\paragraph{Beyond gap-filling.}
The work does close documented Mathlib-facing gaps, but its role in the suite is not
``infrastructure only'': it is a substantive \emph{domain discharge} of the common
collapse/refinement/fiber/obstruction architecture into classical group theory and embedding
problems.

\paragraph{Future work.}
The path toward Mathlib's documented $H^2$ TODO requires: (1)~the $H^2$ bridge
(section cocycle determines a well-defined class in $H^2(G, N)$ depending only on
the equivalence class of the extension); (2)~the inverse construction (twisted
product $N \times_c G$ from a 2-cocycle); (3)~the $H^1$ classification
(conjugacy classes of splittings correspond to $H^1(G, N)$). Each step builds on
the infrastructure established here. The bridge from abstract embedding problems
to actual Galois extensions via Mathlib's \lean{IntermediateField} is a separate
direction. Companion work~\cite{SpivackQuillen} applies the same fiber
architecture to Quillen's Theorem~A for Galois connections, demonstrating that the
pattern reaches beyond group theory into combinatorial topology.
...
Thm.~\ref{thm:hasri} &
  \path{rightHom_surjective_hasRightInverse} &
  \lean{FiberArchitecture.lean} \\
...
Thm.~\ref{thm:split-char} &
  \path{splitting_iff_section_is_homomorphism} &
  \lean{FiberArchitecture.lean} \\
...
Def.~\ref{def:embedding} &
  \lean{EmbeddingProblem}, \lean{toGroupExtension} &
  \lean{EmbeddingProblem.lean} \\

Thm.~\ref{thm:solvable-splits} &
  \lean{solvable\_iff\_splits} &
  \lean{EmbeddingProblem.lean} \\

Thm.~\ref{thm:solvable-cocycle} &
  \lean{solvable\_iff\_trivial\_cocycle} &
  \lean{EmbeddingProblem.lean} \\
...
\lean{InfinityCompression/GeneralMethod/Galois/} (embedding problems),
...
\noindent
\path{FiberArchitecture.lean} is the root; it depends only on\linebreak
\path{Mathlib/GroupTheory/GroupExtension/Basic} and\linebreak
\path{Mathlib/Logic/Function/Basic}.
Three additional modules not discussed in this paper
(\path{NonAbelianObstruction.lean}, \path{SchurMultiplier.lean},
\path{MathlibAdapter.lean}) explore further directions and adapter patterns.
The descent module \path{FaithfullyFlatDescent.lean} is independent, depending
only on\linebreak\path{Mathlib/RingTheory/Flat/FaithfullyFlat/Descent}.
